...
##########################################################Functions
#calculate the start and end position of fragmented contigs and their coordination in scaffolds
...
ggplot(
  ...
  geom_rect(
    ...
    ymin = 0,
    ymax = max(filter(GM,Scaffold == scaffold_H2)$value),
    alpha = 1,
...
fread(recomnination_hap1) %>%  
  ...
  select(Scaffold = V1,
         position = V2,
         value = V3) %>%
reorder_by_coord_position(.,original_coordinates,coordinates) -> REC
...
ggplot(
  ...
  geom_rect(
    aes(NULL,NULL,xmin = start_scaffold, xmax = end_scaffold, fill = contig),
    ymin = log10(min(filter(REC,Scaffold == scaffold_H1)$value)),
    ymax = log10(max(filter(REC,Scaffold == scaffold_H1)$value)),
    alpha = 1,
    data = filter(coordinates,Scaffold == scaffold_H1)
  ) +
  geom_line() +
...
ggplot(
  filter(TELO,
         Scaffold == scaffold_H2),
  aes(x = position,
      y = value)) + 
  geom_rect(
    aes(NULL,NULL,xmin = start_scaffold, xmax = end_scaffold, fill = contig),
    ymin = min(filter(TELO,Scaffold == scaffold_H2)$value),
    ymax = max(filter(TELO,Scaffold == scaffold_H2)$value),
    alpha = 1,
